...
from permutation import Zn_braid
...
def get_secondary_partition(q):
    ...
    r = len(blocks)
    multisets_of_entries = [Counter([q[blocks[i][0]][blocks[m][0]] for m in range(r)]) for i in range(r)]
    for i in range(r):
        appended = False
...
q = [[1,1,-1],[1,1,-1],[-1,-1,1]]
N = 3
q = [[1 for j in range(N)] for i in range(N)]
for i in range(N):
    for j in range(i):
        q[i][j] = random.choice([1, -1])
        q[j][i] = q[i][j]
...
with open("automorphism_test_data.json", "r") as f:
    data = json.load(f)

for i in range(len(data)):
    q = data[i]["input"]
    print("input:", data[i]["input"])
    print("expected:", data[i]["expected"])
